...
export default function DatabaseMetricsPage() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setError(err.message || "Wystąpił błąd podczas pobierania statystyk");
  ...
  if (!dbStats) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 mb-6">Nie udało się pobrać statystyk bazy danych.</p>
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="font-medium">{dbStats.storage.actualUsedStorage || dbStats.storage.usedStorage}</span>
          </div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="w-full bg-gray-200 rounded-full h-2.5">
              <div
                className={`h-2.5 rounded-full ${
                  (dbStats.storage.percentOfFreePlanLimit || dbStats.storage.percentUsed) > 90 
                    ? "bg-red-500" 
                    : (dbStats.storage.percentOfFreePlanLimit || dbStats.storage.percentUsed) > 70 
                      ? "bg-yellow-500" 
                      : "bg-green-500"
                }`}
                style={{ 
                  width: `${dbStats.storage.percentOfFreePlanLimit || dbStats.storage.percentUsed}%` 
                }}
              ></div>
            </div>
            <div className="flex justify-between mt-1 text-xs text-gray-500">
              <span>0%</span>
              <span>{dbStats.storage.percentOfFreePlanLimit || dbStats.storage.percentUsed}% z limitu planu darmowego</span>
              <span>100%</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-2xl font-semibold">{dbStats.documents.collections}</p>
            </div>
            <div>
              <p className="text-gray-600 mb-1">Liczba dokumentów</p>
              <p className="text-2xl font-semibold">{dbStats.documents.documentsCount.toLocaleString()}</p>
            </div>
            <div>
              <p className="text-gray-600 mb-1">Średni rozmiar dokumentu</p>
              <p className="text-2xl font-semibold">{dbStats.documents.avgDocumentSize}</p>
            </div>
            <div>
              <p className="text-gray-600 mb-1">Wolnych miejsc</p>
              <p className="text-2xl font-semibold">
                {((parseFloat(dbStats.storage.totalStorage) - parseFloat(dbStats.storage.usedStorage))).toFixed(2)} MB
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-8">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">Dokumenty w kolekcjach</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {dbStats.documents.documentsByCollection && 
            Object.entries(dbStats.documents.documentsByCollection).map(([collection, count]) => (
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-lg font-semibold text-gray-800">Limity planu MongoDB Atlas</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="border rounded-lg p-4">
            <h3 className="font-medium text-gray-700 mb-3">Przestrzeń dyskowa</h3>
            ...
            <div className="mt-3">
              ...
              <div className="flex justify-between mt-1 text-xs">
                <span className={`${(dbStats.storage.percentOfFreePlanLimit || dbStats.storage.percentUsed) > 90 ? "text-red-600" : "text-gray-500"}`}>
                  {(dbStats.storage.percentOfFreePlanLimit || dbStats.storage.percentUsed) >= 90 ? "Krytycznie mało miejsca!" : ""}
                </span>
                <span className="text-gray-500">
                  {dbStats.storage.percentOfFreePlanLimit || dbStats.storage.percentUsed}% z limitu
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="border rounded-lg p-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {dbStats.planLimits?.connections?.availableConnections || "N/A"}
                </span>
              </div>
            </div>
            <div className="mt-3 p-2 bg-yellow-50 border border-yellow-100 rounded text-xs text-yellow-800">
              <p><strong>Uwaga:</strong> W planie darmowym połączenia są współdzielone z innymi użytkownikami, co może wpływać na wydajność.</p>
            </div>
          </div>
          
          {/* Wydajność */}
          <div className="border rounded-lg p-4">
            <h3 className="font-medium text-gray-700 mb-3">Zasoby obliczeniowe</h3>
            <div className="space-y-2">
              <div className="flex justify-between">
            ...
            <div className="mt-3 p-2 bg-blue-50 border border-blue-100 rounded text-xs text-blue-800">
              <p>Plan darmowy ma ograniczoną wydajność. Przy dużym obciążeniu może wystąpić spowolnienie.</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <h3 className="font-medium text-gray-700 mb-3">Ograniczenia planu darmowego</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <h3 className="font-medium text-green-800 mb-2">Rekomendacje dla planu darmowego</h3>
          <ul className="list-disc pl-5 space-y-1 text-sm text-green-700">
            <li>Regularnie monitoruj wykorzystanie przestrzeni dyskowej - przy zbliżaniu się do limitu 512 MB, rozważ usunięcie niepotrzebnych danych</li>
            <li>Zoptymalizuj zapytania, aby zmniejszyć obciążenie bazy danych</li>
            <li>Dla lepszej wydajności, rozważ przejście na plan płatny, jeśli aplikacja zyska popularność</li>
            <li>Używaj indeksów, ale oszczędnie - zajmują one cenną przestrzeń dyskową</li>
            <li>Unikaj dużych operacji agregacji, które mogą obciążać współdzielone zasoby</li>
          </ul>
        </div>
        ...
        <div className="mt-6 flex items-center justify-between p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div>
            <h3 className="font-medium text-blue-800 mb-1">Potrzebujesz więcej zasobów?</h3>
            <p className="text-sm text-blue-700">
              Plany płatne MongoDB Atlas oferują więcej przestrzeni, dedykowane zasoby i lepszą wydajność.
            </p>
          </div>
          <a 
